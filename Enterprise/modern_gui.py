...
try:
    from version_config import get_version_info, is_feature_available, VERSION_TYPE
except ImportError:
    # Default to Enterprise version if config missing
    def get_version_info():
        return {"type": "ENTERPRISE", "name": "CyberDefense AI Enterprise", "version": "2.1.0", "is_paid": True}
...
class ModernCybersecurityGUI:
    ...
    def create_header(self):
        ...
        plan_color = self.version_info.get('plan_color', '#ff6b35')
        plan_name = self.version_info.get('plan_badge', 'ENTERPRISE')
        
        plan_frame = tk.Frame(title_frame, bg=plan_color, pady=4, padx=12)
        plan_frame.pack(side='left', padx=(15, 0))
        
        plan_label = tk.Label(plan_frame, text=plan_name, 
    ...
    def create_privacy_features_grid(self):
        ...
        # Define all privacy features
        all_features = [
            # Free features - Anti-tracker and Video Access only
            {
                'title': 'Anti-tracker',
                'icon': '🚫',
        ...
        # Filter features based on current version tier
        if VERSION_TYPE == "FREE":
            features = [f for f in all_features if f.get('tier') == 'FREE']
        elif VERSION_TYPE == "TOTAL_SECURITY":
            features = [f for f in all_features if f.get('tier') in ['FREE', 'TOTAL_SECURITY']]
        else:  # ENTERPRISE
